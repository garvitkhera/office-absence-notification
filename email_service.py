...
def send_alert_email(config, alert_date, absent_bearers):
    ...
    msg["Subject"] = f"🔑 Park Agility Office Alert - {formatted_date}"
...
<head>
    <style>
        ...
        .date-box {{ background: white; padding: 20px; border-radius: 8px; text-align: center; margin-bottom: 20px; border-left: 4px solid #1e2a5e; }}
        .date-box h2 {{ margin: 0; color: #1e2a5e; font-size: 20px; }}
        ...
        .status-icon {{ width: 24px; height: 24px; background: #1e2a5e; border-radius: 50%; margin-right: 12px; display: flex; align-items: center; justify-content: center; color: white; font-size: 14px; }}
